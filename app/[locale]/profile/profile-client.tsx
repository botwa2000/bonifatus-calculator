...
export default function ProfileClient({
  ...
  const handleSaveProfile = async (event: React.FormEvent) => {
    ...
    try {
      ...
      if (!res.ok) {
        ...
        setStatusMessage(data.error || t('title'))
      } else {
        setStatusMessage(t('profileUpdated'))
      }
    } catch {
      setStatusMessage(t('title'))
    }
  ...
  const handlePasswordChange = async (event: React.FormEvent) => {
    ...
    if (!newPassword || !confirmPassword) {
      setStatusMessage(ta('passwordsNoMatch'))
      return
    }
    if (newPassword !== confirmPassword) {
      setStatusMessage(ta('passwordsNoMatch'))
    ...
    try {
      ...
      if (!res.ok) {
        const data = await res.json().catch(() => ({}))
        setStatusMessage(data.error || ta('unexpectedError'))
      } else {
      ...
      setStatusMessage(ta('unexpectedError'))
    }
  ...
  const handleThemeChange = async (choice: ThemeChoice) => {
    ...
    try {
      ...
      if (!res.ok) {
        ...
        setStatusMessage(t('themeSet', { theme: choice }))
      }
    } catch {
      setStatusMessage(ta('unexpectedError'))
    }
  ...
  const handleDeleteAccount = async () => {
    ...
    try {
      ...
      const message = error instanceof Error ? error.message : ta('unexpectedError')
        ...
        <div className="grid md:grid-cols-2 gap-6">
          ...
          >
            <div>
              ...
              <p className="text-sm text-neutral-500 dark:text-neutral-400">{t('basicInfoDesc')}</p>
            </div>
          ...
          >
            <div>
              ...
              <p className="text-sm text-neutral-500 dark:text-neutral-400">{t('passwordDesc')}</p>
            </div>
            <label className="block space-y-1">
              <span className="text-sm text-neutral-700 dark:text-neutral-300">
                {t('newPassword')}
            ...
            {passwordStrength && (
              <p
                className={`text-xs ${
                  passwordStrength === ta('strongPassword')
